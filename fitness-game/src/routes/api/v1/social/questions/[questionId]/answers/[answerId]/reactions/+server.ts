...
export const POST: RequestHandler = async (event) => {
  try {
    ...
    const { reactionType } = await parseBody(event, AddAnswerReactionOperationSchema.request.body);
    const { answersService } = event.locals.authServices!;
    await answersService().addReaction({ questionId, answerId, reactionType });
    return noContent();
  } catch (err) {
