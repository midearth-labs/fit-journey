...
import { KnowledgeBaseDetailSchema } from './knowledge-base';

export const QuestionSchema = UUIDSchema.extend(BaseContentSchema.shape).extend({
  knowledge_base_id: KnowledgeBaseDetailSchema.shape.id.describe('The knowledge base ID of the article which this question is based on.'),
  ...
  passage_set_id: KnowledgeBaseDetailSchema.shape.passages.element.shape.id.optional().describe('The passage ID of the passage for which this question is based on. not defined for standalone questions'), // FK to PassageSet (not defined for standalone questions)
});
