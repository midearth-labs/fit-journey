...
import z from 'zod';
import { BaseContentSchema, LearnMoreLinkSchema, AffiliateLinkSchema, ImageSchema, UUIDSchema } from './common';
import { ContentCategorySchema } from './content-category';


export const KnowledgeBaseListSchema = UUIDSchema.extend(BaseContentSchema.shape).extend({
  slug: z.string().describe('The slug of the article. Used as part of the URL of the article and to boost SEO.'),
  content_category_id: ContentCategorySchema.shape.id.describe('The category ID of the article.'),
  ...
  icon: z.string().describe('The lucide icon of the article.'),
...
export const KnowledgeBaseDetailSchema = KnowledgeBaseListSchema.extend({
  ...
  passages: z.array(z.object({
    id: UUIDSchema.shape.id.describe('The ID of the passage. Must be a UUID, unique and immutable.'),
