...
export const QuestionSchema = UUIDSchema.extend(BaseContentSchema.shape).extend({
  knowledge_base_id: z.string().uuid().describe('The knowledge base ID of the article which this question is based on.'),
  ...
  passage_set_id: z.string().optional().describe('The passage ID of the passage for which this question is based on. not defined for standalone questions'), // FK to PassageSet (not defined for standalone questions)
});
